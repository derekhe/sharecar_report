...
def connect():
    return psycopg2.connect(database='sharecar', user='derekhe', password='', host='localhost', port='5432')
    ...
    try:
        with connect() as cnx:
            with gzip.open(file, 'rt') as f:
                print(file)
                crawl_date = datetime.datetime.strptime(os.path.basename(file)[0:-8] + "+0800", "%Y%m%d-%H%M%S%z")
                text = f.read()
                parkings = ujson.loads(text)['parking']

                table_name = get_table_name(file)

                with cnx.cursor() as cursor:
                    for id in parkings.keys():
                        for c in parkings[id]['modelData']['searchList']:
                            cursor.execute(
                                'INSERT INTO ' + table_name + ' (plate_num, position, battery, brand, series, year, name, seats, parking_name, enmileage_integer, is_charging, time) '
...
for file in sorted(glob.glob("/media/derekhe/storage/sharecar/db/gofun/*/**")):
    create_table(file)
